<template>
    <div class="bg-neutral-700">
        ...
        <div><input type="checkbox" name="toggle-mode" :value="createBoard.blackCellsMode" @input="createBoard.toggleMode"></div>
        <div>
            ...
            <div class="bg-white border-2 border-neutral-800 text-neutral-800">
                <div class="flex" v-for="n, i in createBoard.height">
                    <div v-for="m, j in createBoard.width" @click="createBoard.toggleCell(i, j)"
                        class="w-12 h-12 border-neutral-800 border-2 cursor-pointer flex align-stretch justify-center">
                        <div v-if="createBoard.board[i][j].isBlack" class="m-1 w-10 bg-neutral-800"></div>
                        <div v-else>{{createBoard.board[i][j].definitionNumber}}</div>
                    </div>
                </div>
            </div>
        </div>
